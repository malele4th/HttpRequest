...
{
	//判断URL是否有效
	if((strUrl == NULL) || (0 == strcmp(strUrl, ""))) {   // strcmp() : equal,reslut=0
		DebugOut("%s %s %d\tURL为空\n", __FILE__, __FUNCTION__, __LINE__);  // 这几个宏是编译器内置的 
		return 0;
	}
...
char* HttpRequest::HttpDataTransmit(char *strHttpHead, const int iSockFd)
{
	...
	while(1)
	{
		ret = recv(iSockFd, (void *)buf, BUFSIZE,0);  // 客户端通过recv()从服务器接收（响应）数据
...
//检查SocketFd是否为可写不可读状态
int HttpRequest::SocketFdCheck(const int iSockFd)  //一个socket句柄，用文件句柄fd来表示,收发数据
{
